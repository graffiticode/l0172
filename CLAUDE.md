...
### Development
- **Start dev server**: `npm run dev` (starts API server on port 50172 with Firestore emulator)
...
- **Publish package**: `npm run publish` (publishes @graffiticode/l0172)
- **Build lexicon**: `cd packages/api && npm run build-lexicon` (rebuilds language lexicon)
- **Build instructions**: `cd packages/api && npm run build-instructions` (merges basis + l0172 instructions for AI code generation)
...
This is a Graffiticode language implementation (L0172) for creating **FigJam** content. Monorepo with npm workspaces.

### Structure
- **packages/api/**: Express server providing compilation API and language runtime
  - Port: 50172 (dev) or process.env.PORT
...
The compiler extends the @graffiticode/basis framework with L0172-specific logic:
...
- `PORT`: Server port (default: 50172)
